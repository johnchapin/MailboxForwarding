...
- (void) setIds {
    if (self.scanId.length == 0 || self.envelopeId.length == 0) {
        NSString *idsStr = [self.jsonArray objectAtIndex:1];
        NSTextCheckingResult *idsResult = [idsRegex firstMatchInString:idsStr options:0 range:NSMakeRange(0, [idsStr length])];
        self.scanId = [idsStr substringWithRange:[idsResult rangeAtIndex:1]];
        self.envelopeId = [idsStr substringWithRange:[idsResult rangeAtIndex:2]];
    }
}
